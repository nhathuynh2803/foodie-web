...
<script type="text/javascript">
    ...
    function loginClick() {
        ...
        var email = $("#email").val();
        if (email == '') {
            $('#email_required').text('Please Enter Email Id');
            return false;
        }
    ...
    function sendOTP() {
        ...
        if(jQuery("#phone").val() == "")
        {
            ...
            jQuery("#password_required_new1").html("Enter valid phone number.");
        }
        else if (jQuery("#phone").val() && jQuery("#country_selector").val()) {
            var phoneNumber = '+' + jQuery("#country_selector").val() + '' + jQuery("#phone").val();
            database.collection("users").where("phoneNumber", "==", phoneNumber).where("role", "==", 'customer').where('active', '==', true).get().then(async function (snapshots) {
                if (snapshots.docs.length) {
                    var userData = snapshots.docs[0].data();
