...
<script type="text/javascript">
    ...
    async function getVendorDetails(vendorID) {
        ...
        vendorDetailsRef.get().then(async function (vendorSnapshots) {
            ...
            $("#seller-image").html('<a href="' + view_vendor_details + '"><img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'" style="height: 65px; width: 65px; border-radius: 50%" src="' + photo + '"></a>');
    ...
    function buildVendorProductsHTML(snapshots) {
        ...
        alldata.forEach((listval) => {
            ...
            html = html + '<img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'"  src="' + photo + '">';
            html = html + '</div>';
        ...
        allreviewdata.forEach((listval) => {
            ...
            if (val.photos.length > 0) {
                ...
                $.each(val.photos, function (key, img) {
                    photos += '<li><img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'" src="' + img + '" width="100"></li>';
                });
                photos += '</ul></div>';
            }
    ...
    async function buildHTML(snapshots) {
        ...
        if (vendorProduct != undefined) {
            ...
            var price = getPrice(parseFloat(vendorProduct.price));
            if (vendorProduct.hasOwnProperty('disPrice') && vendorProduct.disPrice != '0') {
                price = getPrice(parseFloat(vendorProduct.disPrice));
            }
            ...
            if (vendorProduct.photos != null && vendorProduct.photos.length > 0) {
                ...
                vendorProduct.photos.forEach((photo) => {
                    ...
                    html = html + '<img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'" alt="#" src="' + photo + '" class="img-fluid item-img w-100">';
                    html = html + '</div>';
                });
                html = html + '</div>';
            ...
            if (vendorProduct.item_attribute != null && vendorProduct.item_attribute != "" && vendorProduct.item_attribute.attributes.length > 0 && vendorProduct.item_attribute.variants.length > 0) {
                ...
                if (vendorProduct.hasOwnProperty('disPrice') && vendorProduct.disPrice != '' && vendorProduct.disPrice != '0' && vendorProduct.item_attribute == null) {
                    var or_price = getFormattedPrice(parseFloat(vendorProduct.price));
				    var dis_price = getFormattedPrice(parseFloat(vendorProduct.disPrice));
            ...
            var disPrice = getPrice(parseFloat(vendorProduct.disPrice));
    ...
    function buildHTMLRelatedProducts(snapshots) {
        ...
        alldata.forEach((listval) => {
            ...
            html = html + '<a href="' + view_vendor_details + '"><img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'" alt="#" src="' + photo + '" class="img-fluid item-img w-100"></a></div><div class="py-2 position-relative"><div class="list-card-body position-relative"><h6 class="product-title mb-1"><a href="' + view_vendor_details + '" class="text-black">' + val.name + '</a></h6>';
    ...
    async function getVariantsHtml(vendorProduct, attributes, variants) {
        ...
        if (variation_sku) {
            ...
            if (variant_info.length > 0) {
                ...
                var variant_price = getPrice(parseFloat(variant_info[0].variant_price));
