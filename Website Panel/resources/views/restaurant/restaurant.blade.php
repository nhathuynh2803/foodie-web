...
<script type="text/javascript">
    ...
    async function getVendorDetails() {
        ...
        vendorDetailsRef.get().then(async function (vendorSnapshots) {
            ...
            $("#restaurant-pic").html('<img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'" alt="#" class="restaurant-pic" src="' + photo + '">');
            ...
            if (vendorDetails.photos.length > 0) {
                ...
                if (vendorDetails.photos[0]!="" && vendorDetails.photos[0]!=null) {
                    gallery += '<img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'" src="' + vendorDetails.photos[0] + '" class="banner-small-pic">';
                } else {
                ...
                if (vendorDetails.photos[1]!="" && vendorDetails.photos[1]!=null) {
                    gallery += '<img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'" src="' + vendorDetails.photos[1] + '" class="banner-small-pic">';
                } else {
                    gallery += '<img src="' + placeholderImageSrc + '" class="banner-small-pic">';
                }
                gallery += '</div>';
                ...
                if (vendorDetails.photos[2]!="" && vendorDetails.photos[2]!=null) {
                    gallery += '<img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'" src="' + vendorDetails.photos[2] + '" class="banner-small-pic">';
                } else {
                    gallery += '<img src="' + placeholderImageSrc + '" class="banner-small-pic">';
                }
                gallery += '</div>';
                ...
                $.each(vendorDetails.photos, function (key, value) {
                    popup_gallery += '<a href="' + value + '"><img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'" src="' + value + '"></a>';
                });
            ...
            html_rating = html_rating + '<li class="rating__item"></li>';
            html_rating = html_rating + '<li class="rating__item"></li>';
            html_rating = html_rating + '</ul><p class="label-rating ml-2 small" id="vendor_reviews">(' + reviewsCount + ' {{trans("lang.review")}})</p>';
    ...
    async function getCouponDetails() {
        var date = new Date();
        var couponRef = database.collection('coupons').where('isEnabled', '==', true).where("isPublic",'==',true).where("resturant_id", "==", vendorId).where('expiresAt', '>=', date);
    ...
    function buildProductsHTML(snapshots) {
        ...
        alldata.forEach((listval) => {
            ...
            html = html + '<div class="member-plan position-absolute"><span class="badge badge-dark ' + statusclass + '">' + status + '</span></div><a href="' + view_vendor_details + '"><img onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'" alt="#" src="' + photo + '" class="img-fluid item-img w-100"></a></div><div class="py-2 position-relative"><div class="list-card-body position-relative"><h6 class="product-title mb-1"><a href="' + view_vendor_details + '" class="text-black">' + val.name + '</a></h6>';
